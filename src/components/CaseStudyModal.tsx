...
import { X, CheckCircle2, Lightbulb, Compass, Quote } from 'lucide-react';
      ...
      >
        ...
        <div className="p-4 sm:p-6 border-b border-white/10 bg-zinc-900/80 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-zinc-400 font-mono hidden sm:inline">
              Client: {caseStudy.client} • {caseStudy.year}
            </span>
          </div>
        ...
        <div className="px-6 border-b border-white/10 bg-zinc-900/40 flex gap-2 sm:gap-6 overflow-x-auto shrink-0 py-2">
          {[
            ...
            { id: 'research' as ModalTab, label: '2. UX Research' },
            { id: 'process' as ModalTab, label: '3. Design Evolution' },
            { id: 'impact' as ModalTab, label: '4. Impact & Results' },
          ].map((tab) => (
            <button
              key={tab.id}
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-10">
          ...
          <div className="space-y-4 max-w-3xl">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3">
              <div className="bg-zinc-900/60 p-3 rounded-lg border border-white/5 space-y-0.5">
              ...
              <div className="bg-zinc-900/60 p-3 rounded-lg border border-white/5 space-y-0.5">
                <div className="text-[10px] text-zinc-500 font-mono uppercase">Duration</div>
                <div className="text-xs font-semibold text-zinc-200">{caseStudy.duration}</div>
              </div>
              <div className="bg-zinc-900/60 p-3 rounded-lg border border-white/5 space-y-0.5">
                <div className="text-[10px] text-zinc-500 font-mono uppercase">Platform</div>
                <div className="text-xs font-semibold text-zinc-200">Web &amp; Mobile Responsive</div>
              </div>
              <div className="bg-zinc-900/60 p-3 rounded-lg border border-white/5 space-y-0.5">
                <div className="text-[10px] text-zinc-500 font-mono uppercase">Primary Metric</div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {caseStudy.overview.myRole.map((item, idx) => (
                    <div key={idx} className="flex items-start gap-2 text-xs sm:text-sm text-zinc-300 bg-zinc-900/30 p-3 rounded-lg border border-white/5">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {caseStudy.overview.tools.map((tool) => (
                    <span key={tool} className="px-3 py-1 rounded bg-zinc-800 text-zinc-300 text-xs font-medium border border-white/10">
                      {tool}
                    </span>
                  ))}
                </div>
          ...
          {activeTab === 'research' && (
            <div className="space-y-8 animate-in fade-in duration-200">
              <div className="p-5 rounded-xl bg-zinc-900/50 border border-white/10 space-y-2">
                <div className="text-xs font-mono text-zinc-400 uppercase">Target User Archetype</div>
              ...
              <div className="space-y-4">
                <h3 className="text-base font-semibold text-white">
                  Key Qualitative &amp; Quantitative Insights
                </h3>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {caseStudy.research.problemStatements.map((hmw, idx) => (
                    <div key={idx} className="p-3.5 rounded-lg bg-zinc-900/40 border border-white/5 text-xs sm:text-sm text-zinc-300 font-medium italic">
                      &ldquo;{hmw}&rdquo;
                    </div>
                  ))}
                </div>
            ...
            <div className="space-y-8 animate-in fade-in duration-200">
              <div className="space-y-6">
                {caseStudy.designProcess.map((step, idx) => (
                  <div key={idx} className="p-5 rounded-xl bg-zinc-900/80 border border-white/10 space-y-3">
                    ...
                    <div className="pt-2 flex flex-wrap gap-2">
                      {step.highlights.map((highlight, hIdx) => (
                        <span key={hIdx} className="px-2.5 py-1 rounded bg-zinc-800/80 text-[11px] text-zinc-300 border border-white/5">
                          ✓ {highlight}
                        </span>
                      ))}
                    </div>
        ...
        <div className="p-4 sm:p-6 border-t border-white/10 bg-zinc-900/90 flex items-center justify-between shrink-0">
          <div className="text-xs text-zinc-400 font-mono hidden sm:inline">
            Press <kbd className="px-1.5 py-0.5 bg-zinc-800 border border-white/10 rounded text-[10px]">Esc</kbd> or click outside to exit
          </div>
          <button
            onClick={onClose}
